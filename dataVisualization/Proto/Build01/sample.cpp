...
{
	//test datapoints
	addData(&incomingData, 1.3, 1.2, 1);
	addData(&incomingData, 2, 1.9, 1.2);
	addData(&incomingData, 2.5, 2, 0.7);
	addData(&incomingData, 2.7, 1.0, 1);
	addData(&incomingData, 1.4, -0.5, 0.5);
	addData(&incomingData, 1.2, 0.7, 1);
	/*
...
Display( )
{
	...
	iterateListCircle(incomingData); //visualize data
	iterateListLine(incomingData);
...
iterateListCircle(data *item)
{
	...
	while (item != NULL)
	{
		...
		drawCircle(item->latitude - latitudeConst, item->longitude - longitudeConst, item->frequency, depth);
		item = item->next;
	}
	return;
}
...
iterateListLine(data *item)
{
	...
	while (item != NULL)
	{
		glColor3f(1., 1., 1.);
		depth = depth + 0.00001;
		glVertex3f(item->latitude - latitudeConst, item->longitude - longitudeConst, depth);
		item = item->next;
	}
	glEnd();
	glEnable(GL_TEXTURE_2D);
